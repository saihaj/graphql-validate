...
const GRAPHQL_ERROR_GUIDE_LINKS_MAP: [
  rule_message_regex: RegExp,
  link: string,
][] = [
  [
    /Cannot query field "(.*)" on type "(.*)"/,
    'https://thegraph.com/docs/en/#UnknownX',
  ],
  ...
  [
    ...
    'https://thegraph.com/docs/en/#LoneAnonymousOperationRule',
  ],
  [
    /Fragment "(.*)" is never used/,
    'https://thegraph.com/docs/en/#NoUnusedFragmentsRule',
  ],
  [
    /Variable "(.*)" is never used/,
    'https://thegraph.com/docs/en/#NoUnusedVariablesRule',
  ],
  [
    /Fields "(.*)" conflict because (.*)\. Use different aliases on the fields to fetch both if this was intentional\./,
    'https://thegraph.com/docs/en/#OverlappingFieldsCanBeMergedRule',
  ],
  [
    /Fragment cannot condition on non composite type "(.*)"/,
    'https://thegraph.com/docs/en/#FragmentsOnCompositeTypesRule',
  ],
  [
    /Fragment cannot be spread here as objects/,
    'https://thegraph.com/docs/en/#PossibleFragmentSpreadsRule',
  ],
  [
    /Field "(.*)" must not have a selection since type "(.*)" has no subfields./,
    'https://thegraph.com/docs/en/#ScalarLeafsRule',
  ],
  [
  ...
  [
    ...
    'https://thegraph.com/docs/en/#UniqueArgument',
  ],
  [
  ...
  [
    ...
    'https://thegraph.com/docs/en/#KnownDirectivesRule',
  ],
  [
    /The directive "(.*)" can only be used once at this location/,
    'https://thegraph.com/docs/en/#UniqueDirectivesPerLocationRule',
  ],
  [
    /There can be only one fragment named "(.*)"/,
    'https://thegraph.com/docs/en/#UniqueFragmentNamesRule',
  ],
  [
    /There can be only one operation named "(.*)"/,
    'https://thegraph.com/docs/en/#UniqueOperationNamesRule',
  ],
  [
    /There can be only one variable named "(.*)"/,
    'https://thegraph.com/docs/en/#UniqueVariableNamesRule',
  ],
  [
    /Variable "(.*)" of type "(.*)" used in position expecting type "(.*)"/,
    'https://thegraph.com/docs/en/#VariablesInAllowedPositionRule',
  ],
]
...
const logError = (
  ...
  const errorGuideLink = GRAPHQL_ERROR_GUIDE_LINKS_MAP.find(([regexp]) =>
  ...
  ${
    errorGuideLink
      ? `\n More information at ${terminalLink(
          errorGuideLink[1],
          errorGuideLink[1],
          { fallback: false },
        )}`
...
const validateOperations = async (
  ...
  documents.forEach((document) => {
    const errors = validate(schema, document.document!, rules)
...
const defaultRules = specifiedRules.concat(NoDeprecatedCustomRule)
...
const main = async () => {
  program
    ...
    .option(
      '-r, --rules <Rule1,Rule2,...,RuleN>',
      'The list of rules to apply for the validation',
    )
    .parse(process.argv)

  const schema = program.opts().schema
  const operation = program.opts().operation
  const rules = (program.opts().rules || '').split(',').filter(Boolean)

  await validateOperations(
    schema,
    operation,
    defaultRules.filter((f) => rules.length === 0 || rules.includes(f.name)),
  )
}
